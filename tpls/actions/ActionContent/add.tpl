...
<form action="" method="POST" enctype="multipart/form-data" id="form-topic-add" class="wrapper-content form-actions">
    ...
    <p><label for="blog_id">{$aLang.topic_create_blog}</label>
        ...
        <small class="help-block">{$aLang.topic_create_blog_notice}</small>
    </p>
    ...
    <p><label for="topic_title">{$aLang.topic_create_title}:</label>
        ...
        <small class="help-block">{$aLang.topic_create_title_notice}</small>
</p>
...
<span class="b-topic_url_demo">{$aEditTopicUrl.after}</span>
            ...
            <button class="button js-tip-help" title="{$aLang.topic_create_url_short}" 
                   onclick="ls.topic.shortUrl('{$_aRequest.topic_url_short}'); return false;">
                   <i class="icon-share-alt"></i></button>
    ...
    {if $oContentType->isAllow('link')}
        ...
        <div class="poll-create" id="topic-link" {if !$_aRequest.topic_link_url}style="display:none;"{/if}>
            <p><label for="topic_link_url">{$aLang.topic_link_create_url}:</label>
                ...
                <small class="help-block">{$aLang.topic_link_create_url_notice}</small></p>
        </div>
    {/if}

    <p><label for="topic_tags">{$aLang.topic_create_tags}:</label>
        <input type="text" id="topic_tags" name="topic_tags" value="{$_aRequest.topic_tags}" 
               class="input-block-level autocomplete-tags-sep" />
        <small class="help-block">{$aLang.topic_create_tags_notice}</small>
    </p>

    {hook run='form_add_content'}

    <p><label class="checkbox"><input type="checkbox" id="topic_forbid_comment" name="topic_forbid_comment" class="input-checkbox" 
              value="1" {if $_aRequest.topic_forbid_comment==1}checked{/if} />
            {$aLang.topic_create_forbid_comment}</label>
        <small class="help-block">{$aLang.topic_create_forbid_comment_notice}</small>
    </p>

    {if $oUserCurrent->isAdministrator()}
        <p><label class="checkbox"><input type="checkbox" id="topic_publish_index" name="topic_publish_index" class="input-checkbox" 
                  value="1" {if $_aRequest.topic_publish_index==1}checked{/if} />
                {$aLang.topic_create_publish_index}</label>
            <span class="help-block"><small>{$aLang.topic_create_publish_index_notice}</small></span>
</p>
    {/if}
...
{if $oTopic AND $oTopic->getPublish()}
    ...
    <button onclick="ls.topic.preview('form-topic-add','text_preview'); return false;" 
            type="submit" name="submit_preview" class="btn">
    {$aLang.topic_create_submit_preview}
    </button>
    <button type="submit" name="submit_topic_save" id="submit_topic_save" class="btn">
{if $oTopic AND $oTopic->getPublish()}
